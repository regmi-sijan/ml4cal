#!/usr/bin/env python
##########################################################
# Load a model, a data file and compare the model's
...
X = dataset[:,0:L]
y = dataset[:,L] # the "y" vector: amplitude, time, pedestal
end = time.time()

#for i in range(0,10): print(X[i], '!', y[i])
#exit(0)
...
print(np.average(answer), np.average(diff))
